...
class InputDataUserController extends Controller
{
    ...
    public function save(Request $request)
    {
        try {
            ...
            $data_pemilik = DB::table('data_pemilik')->where("user_id", "=", Auth::user()->id)->get();
            if (count($data_pemilik) < 1) {
            ...
            } else { //apabila data pemilik >1 maka data pemilik akan terupdat e menjadi data yang terakhir diinputkan
                $update = DB::table('data_pemilik')->where("user_id", "=", Auth::user()->id)->update([
                    'nik' => $request->input('nik'),
                    'nama' => $request->input('nama'),
                    'alamat' => $request->input('alamat'),
                    'rt' => $request->input('rt'),
            ...
            //mengambil data yiatu keterampilan dan jumlahnya untuk di simpan dalam perusahaan_keterampilan. Data ini bersifat array karena user dapat menginputkan banyak data.
            ...
            //mengambil data yaitu id_sertifikat, nomor, dan dokumennya untuk disimpan di tabel perusahaan_sertifikat. Data ini bersifat array karena user dapat menginputkan banyak data. 
            ...
            //if (!$data_pemilik || !$data_perusahaan || !$data_fasilitas) {
            DB::commit();
            //}
        } catch (Exception $e) {
            DB::rollBack();
    ...
    //menampilkan data yang telah diinput oleh user saja dengan status 0 (belum diverifikasi)
    public function data()
    {
        $data['pemilik'] = DataPemilik::where(['user_id' => Auth::user()->id])->get();
        $data['perusahaan'] = [];
        if (count($data['pemilik']) > 0) {
            $data['perusahaan'] = DB::select("select * from data_perusahaan where id_pemilik = :id_pemilik", ["id_pemilik" => $data['pemilik'][0]->id_pemilik]);
        }
   ...
   //menampilkan detail dari data yang telah diinput
    public function show($id_perusahaan)
    {
        ...
        // $data = $data[0];
        // dd($data_keterampilan);die;
        return view('detailUser', compact('data', 'data_pemilik', 'data_keterampilan', 'data_bpjs', 'data_sertifikat', 'data_pameran', 'data_pelatihan', 'data_bantuan_alat', 'data_bantuan_bahan'));
    }
        //fungsi edit akan menampilkan data yang telah diinput yang nantinya user dapat melakukan edit (ubah data). data disimpan dalam array agar memudahkan pemanggila
    ...
    public function saveEdit(Request $request, $id_perusahaan)
    {
        try {
            ...
            $data_perusahaan = DB::table('data_perusahaan')->where("id_perusahaan", "=", $request->input('id_perusahaan'))->update([
                'id_pemilik' => $data_pemilik, 
                'foto_perusahaan' => $namafile==null ? $data_lama_perusahaan->foto_perusahaan : $namafile,
                'badan_usaha' => $request->input('badan_usaha'),
                'nama_perusahaan' => $request->input('nama_perusahaan'),
                'alamat_perusahaan' => $request->input('alamat_perusahaan'),
                'kecamatan_perusahaan' => $request->input('kecamatan_perusahaan'),
 ...
 //menampilkan data statistik
    ...
    {   
        ...
        $data['data'] = DB::table('data_perusahaan')
            ...
            ->paginate(10);
        return view('statistikUser', $data);
    }
